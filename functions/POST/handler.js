...
const updateApplication = require('./dynamo').updateApplication;
...
exports.handler = function(evt, cxt, cb) {
    if (evt) {
        ...
        if (evt.actions[0].value === 'accept') {
            invite(evt.callback_id).then(() => {updateApplication(evt.callback_id, evt.actions[0].value, evt.user.name);}).then(() => {
            ...
            updateApplication(evt.callback_id, evt.actions[0].value, evt.user.name).then(() => {
